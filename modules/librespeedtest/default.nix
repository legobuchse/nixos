{ lib, pkgs, config, ... }:
with lib;
let cfg = config.simon.librespeedtest;
in
{

  options.simon.librespeedtest = {
    ...
    domain = mkOption {
      type = types.str;
      default = "speed.example.com";
      description = "domain name for librespeedtest";
    };
  ...
  config = mkIf cfg.enable (mkMerge [
    ...
    (mkIf cfg.enableNginx {
      simon.nginx.enable = true;
      simon.dyndns = mkIf cfg.enableDyndns {
        enable = true;
        domains = [ cfg.domain ];
      };
